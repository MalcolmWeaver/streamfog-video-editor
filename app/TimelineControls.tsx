'use client';
import React, { useState, useEffect, useRef } from 'react';
import { AppliedFilter, LensOption, TimelineControlsProps } from '../types';

/**
 * A component to display and control the video timeline, including scrubbing and filter visualization.
 *
 * @param {TimelineControlsProps} props
 */
const TimelineControls: React.FC<TimelineControlsProps> = ({
    duration,
    currentTime,
    onScrub,
    isPlaying,
    onPlayPause,
    onAddNewFilter,
    onUpdateFilter,
    onDeleteFilter,
    appliedFilters = [],
    availableLensOptions = []
}: TimelineControlsProps) => {
    // State for the "Add New Filter" UI
    const [isAddingFilter, setIsAddingFilter] = useState<boolean>(false);
    const [newFilterStart, setNewFilterStart] = useState<number>(0);
    const [newFilterEnd, setNewFilterEnd] = useState<number>(0);
    const [newFilterLabel, setNewFilterLabel] = useState<string>('New Filter');

    const [selectedLensId, setSelectedLensId] = useState<string>(availableLensOptions[0]?.id || '');
    const [selectedLensColor, setSelectedLensColor] = useState<string>(availableLensOptions[0]?.color || '#3B82F6');

    // State for managing existing filters
    const [showManageFilters, setShowManageFilters] = useState<boolean>(false);
    const [editingFilterId, setEditingFilterId] = useState<string | null>(null);
    const [editFilterStart, setEditFilterStart] = useState<number>(0);
    const [editFilterEnd, setEditFilterEnd] = useState<number>(0);
    const [editFilterLabel, setEditFilterLabel] = useState<string>('');
    const [editFilterLensId, setEditFilterLensId] = useState<string>('');
    // Ref to the scrubber input element for focus management if needed
    const scrubberRef = useRef<HTMLInputElement>(null);

    useEffect(() => {
        if (availableLensOptions.length > 0 && !selectedLensId) {
            setSelectedLensId(availableLensOptions[0].id);
            setSelectedLensColor(availableLensOptions[0].color || '#3B82F6');
        }
    }, [availableLensOptions, selectedLensId]);

    // Update newFilterEnd when duration changes, if it exceeds duration
    useEffect(() => {
        if (newFilterEnd > duration) {
            setNewFilterEnd(duration);
        }
        if (newFilterStart > duration) {
            setNewFilterStart(duration);
        }
        if (editingFilterId && editFilterEnd > duration) {
            setEditFilterEnd(duration);
        }
        if (editingFilterId && editFilterStart > duration) {
            setEditFilterStart(duration);
        }
    }, [duration, newFilterEnd, newFilterStart, editingFilterId, editFilterEnd, editFilterStart]);


    // Helper to format time into MM:SS (or HH:MM:SS for longer videos)
    const formatTime = (seconds: number): string => {
        if (isNaN(seconds) || seconds < 0) return '00:00';
        const date = new Date(null as any); // Type assertion for Date constructor
        date.setSeconds(seconds);
        const result = date.toISOString().substr(11, 8); // HH:MM:SS
        return result.startsWith('00:') ? result.substr(3) : result; // Remove HH: if zero
    };

    // Handle scrubber change
    const handleScrubberChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        const newTime = parseFloat(event.target.value);
        onScrub(newTime);
    };

    // Handle adding a new filter
    const handleAddFilter = () => {
        setIsAddingFilter(true);
        // Initialize with current time or some defaults
        setNewFilterStart(parseFloat(currentTime.toFixed(2)));
        setNewFilterEnd(parseFloat(Math.min(currentTime + 5, duration).toFixed(2))); // Default to 5s length or end of video
        setNewFilterLabel('New Filter');
        if (availableLensOptions.length > 0) {
            setSelectedLensId(availableLensOptions[0].id);
            setSelectedLensColor(availableLensOptions[0].color || '#3B82F6');
        } else {
            setSelectedLensId('');
            setSelectedLensColor('#3B82F6');
        }
    };

    // Handle saving the new filter
    const handleSaveFilter = () => {
        // Basic validation
        if (newFilterStart >= newFilterEnd) {
            // Using a simple alert for now, but replace with a custom modal UI in production
            alert('Start time must be before end time!');
            return;
        }
        if (newFilterStart < 0 || newFilterEnd > duration) {
            alert('Times are out of video bounds!');
            return;
        }

        onAddNewFilter({
            id: `filter-${Date.now()}`, // Simple unique ID
            start: newFilterStart,
            end: newFilterEnd,
            label: newFilterLabel,
            color: selectedLensColor,//`#${Math.floor(Math.random() * 16777215).toString(16).padStart(6, '0')}`, // Random color for visualization
            filterId: selectedLensId 
        });
        setIsAddingFilter(false); // Hide the input form
    };

    // Handle canceling new filter addition
    const handleCancelAddFilter = () => {
        setIsAddingFilter(false);
    };

    const handleEditExistingFilter = (filter: AppliedFilter) => {
        setEditingFilterId(filter.id);
        setEditFilterStart(filter.start);
        setEditFilterEnd(filter.end);
        setEditFilterLabel(filter.label);
        setEditFilterLensId(filter.filterId);
        setIsAddingFilter(false); // Hide add section when managing
    };

    const handleSaveEditFilter = () => {
        if (!editingFilterId) return;

        if (editFilterStart >= editFilterEnd) {
            alert('Start time must be before end time!');
            return;
        }
        if (editFilterStart < 0 || editFilterEnd > duration) {
            alert('Times are out of video bounds!');
            return;
        }

        onUpdateFilter({
            id: editingFilterId,
            start: editFilterStart,
            end: editFilterEnd,
            label: editFilterLabel,
            color: availableLensOptions.find(l => l.id === editFilterLensId)?.color || appliedFilters.find(f => f.id === editingFilterId)?.color || '#3B82F6',
            filterId: editFilterLensId 
        });
        setEditingFilterId(null); // Exit editing mode
    };

    const handleCancelEditFilter = () => {
        setEditingFilterId(null);
    };

    const handleDeleteExistingFilter = (id: string) => {
        if (window.confirm('Are you sure you want to delete this filter?')) {
            onDeleteFilter(id);
            if (editingFilterId === id) { 
                setEditingFilterId(null);
            }
        }
    };

const [startInput, setStartInput] = useState<string | null>(null);
const [endInput, setEndInput] = useState<string | null>(null);

const [startError, setStartError] = useState('');
const [endError, setEndError] = useState('');


function isValidFloat(str: string): boolean {
  return /^(\d+)(\.\d{0,})?$/.test(str.trim());
}
function formatFloat(num: number): string {
  return num.toFixed(2);
}
const [editStartInput, setEditStartInput] = useState<string>(editFilterStart.toFixed(2));
const [editEndInput, setEditEndInput] = useState<string>(editFilterEnd.toFixed(2));

const [editStartError, setEditStartError] = useState('');
const [editEndError, setEditEndError] = useState('');

useEffect(() => {
  setEditStartInput(editFilterStart.toFixed(2));
  setEditEndInput(editFilterEnd.toFixed(2));
  setEditStartError('');
  setEditEndError('');
}, [editingFilterId, editFilterStart, editFilterEnd]);


    return (
        <div className="w-full mt-6 p-4 bg-gray-700 rounded-lg flex flex-col gap-4">
          {/* Play/Pause Button and Current Time/Duration Display */}
          <div className="flex items-center justify-between text-gray-300">
            <button
              onClick={onPlayPause}
              className="p-2 rounded-full bg-purple-600 hover:bg-purple-700 text-white focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-opacity-75 transition-colors duration-200"
              aria-label={isPlaying ? 'Pause video' : 'Play video'}
            >
              {isPlaying ? (
                <svg className="h-6 w-6" fill="currentColor" viewBox="0 0 24 24"><path d="M6 19h4V5H6v14zm8-14v14h4V5h-4z"/></svg> // Pause icon
              ) : (
                <svg className="h-6 w-6" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg> // Play icon
              )}
            </button>
            <div className="text-lg font-mono">
              <span>{formatTime(currentTime)}</span> / <span>{formatTime(duration)}</span>
            </div>
          </div>

          {/* Scrubber Input */}
          <input
            ref={scrubberRef}
            type="range"
            min="0"
            max={duration}
            value={currentTime}
            onChange={handleScrubberChange}
            // Custom Tailwind styling for the thumb (slider handle)
            className="w-full h-2 bg-gray-600 rounded-lg appearance-none cursor-pointer
                             [&::-webkit-slider-thumb]:w-4 [&::-webkit-slider-thumb]:h-4 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-purple-500 [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:shadow-lg
                             [&::-moz-range-thumb]:w-4 [&::-moz-range-thumb]:h-4 [&::-moz-range-thumb]:rounded-full [&::-moz-range-thumb]:bg-purple-500 [&::-moz-range-thumb]:appearance-none [&::-moz-range-thumb]:shadow-lg
                             focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-opacity-75"
            aria-label="Video scrubber"
          />

          {/* Filter/Lens Time Ranges Visualization */}
          <div className="relative w-full h-8 bg-gray-600 rounded-md overflow-hidden my-2">
            {/* Render existing applied filters */}
            {appliedFilters.map((filter) => (
              <div
                key={filter.id}
                className="absolute h-full rounded-sm opacity-75 group cursor-pointer flex items-center justify-center text-xs font-bold text-white overflow-hidden whitespace-nowrap"
                style={{
                  left: `${(filter.start / duration) * 100}%`,
                  width: `${((filter.end - filter.start) / duration) * 100}%`,
                  backgroundColor: filter.color || '#3B82F6',
                }}
                title={`${filter.label}: ${formatTime(filter.start)} - ${formatTime(filter.end)} (Lens ID: ${filter.filterId})`}
                onClick={() => handleEditExistingFilter(filter)} // Click to edit
              >
                <span className="group-hover:block hidden px-1 py-0.5 bg-gray-900 bg-opacity-75 rounded-sm">Edit</span>
              </div>
            ))}
            {/* Current Playhead Indicator */}
            <div
              className="absolute top-0 bottom-0 w-1 bg-purple-400 z-10"
              style={{ left: `${(currentTime / duration) * 100}%` }}
            ></div>
          </div>

          {/* "Add New Filter" Button / Form */}
      {!isAddingFilter ? (
        <button
          onClick={handleAddFilter}
          className="mt-4 w-full py-2 px-4 bg-blue-600 hover:bg-blue-700 rounded-lg text-white font-semibold transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-75"
        >
          Add New Filter
        </button>
      ) : (
        <div className="mt-4 p-4 bg-gray-800 rounded-lg shadow-inner flex flex-col gap-4 border border-gray-700"> {/* Changed bg to gray-800, added border */}
          <h3 className="text-xl font-bold text-purple-300 mb-2">Define New Filter Slice</h3> {/* Made title bolder, purple */}
          
          <div className="flex flex-col gap-2 bg-gray-700 p-3 rounded-lg border border-gray-600"> {/* Added bg, padding, border */}
            <label className="text-gray-300 font-medium text-lg">Select A Lens</label> {/* Larger, medium font */}
            {availableLensOptions.length > 0 ? (
                <div className="flex overflow-x-auto gap-4 py-2 px-1 custom-scrollbar"> {/* Increased gap, added custom-scrollbar class */}
                {availableLensOptions.map((lens) => (
                    <button
                    key={lens.id} // Use lens.id as key, assuming unique
                    onClick={() => {
                        setSelectedLensId(lens.id);
                        setSelectedLensColor(lens.color || '#3B82F6');
                    }}
                     className={`flex-shrink-0 w-28 h-28 rounded-xl flex flex-col items-center justify-center text-center text-sm font-medium border-2 transition-all duration-200 relative overflow-hidden shadow-md
                        ${selectedLensId === lens.id ? 'border-purple-400 ring-2 ring-purple-400 scale-105' : 'border-gray-600 hover:border-purple-300'}
                      `}
                      style={{ backgroundColor: lens.color || '#3B82F6' }}
                    >
                      {lens.thumbnailUrl ? (
                        <>
                          <img src={lens.thumbnailUrl} alt={lens.name} className="absolute inset-0 w-full h-full object-cover" />
                          <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black via-black/60 to-transparent p-2 text-gray-200 font-semibold text-xs leading-tight">
                            {lens.name}
                          </div>
                        </>
                      ) : (
                        <>
                          {/* Icon placeholder - could be an SVG or FontAwesome icon */}
                          <div className="mb-1 text-2xl text-white">
                            {/* Example: a simple circle icon based on color */}
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                              <circle cx="12" cy="12" r="10" />
                            </svg>
                          </div>
                          <span className="text-white font-semibold text-base px-1">{lens.name}</span> {/* Bolder, larger text */}
                        </>
                      )}
                      {/* Overlay for selected state */}
                      {selectedLensId === lens.id && (
                        <div className="absolute inset-0 bg-purple-500 opacity-65 rounded-xl flex items-center justify-center pointer-events-none">
                          <svg className="w-8 h-8 text-white opacity-90" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7"></path>
                          </svg>
                        </div>
                      )}
                    </button>
                ))}
                </div>
            ) : (
                <p className="text-gray-400 text-sm p-2">No lenses available. Check API token and group ID.</p>
            )}
            
            {selectedLensId && (
                <p className="text-sm text-gray-400 mt-2">Selected: <span className="font-semibold text-purple-300">{availableLensOptions.find(l => l.id === selectedLensId)?.name || selectedLensId}</span></p>
            )}
          </div>
          {/* END Filter/Lens Carousel */}

          <div className="flex items-center gap-2">
            <label htmlFor="new-filter-label" className="text-gray-300 w-20">Label:</label>
            <input
              type="text"
              id="new-filter-label"
              value={newFilterLabel}
              onChange={(e: React.ChangeEvent<HTMLInputElement>) => setNewFilterLabel(e.target.value)}
              className="flex-1 p-2 rounded-md bg-gray-700 border border-gray-500 text-white focus:outline-none focus:ring-2 focus:ring-blue-500"
              placeholder="e.g., 'Face Blur'"
            />
          </div>
        {/* START INPUT */}
        <div className="flex flex-col sm:flex-row sm:items-center gap-2 w-full">
          <label htmlFor="new-start-time" className="text-gray-300 sm:w-20 w-full">Start:</label>
          <input
            type="text"
            id="new-start-time"
            value={startInput ?? ''}
            placeholder="0.00"
            onChange={(e) => {
              setStartInput(e.target.value);
              setStartError('');
            }}
            onBlur={() => {
              const raw = startInput?.trim();
              if (!raw || !isValidFloat(raw)) {
                setStartError('Invalid time format');
                return;
              }

              const parsed = parseFloat(raw);
              const fallbackEnd = endInput ? parseFloat(endInput) : parsed + 5;

              if (parsed >= fallbackEnd) {
                setStartError('Start must be less than End');
                return;
              }

              setStartInput(formatFloat(parsed));
              setNewFilterStart(parsed);

              if (!endInput) {
                const end = parsed + 5;
                setNewFilterEnd(end);
                setEndInput(formatFloat(end));
              }
            }}
            className={`flex-1 p-2 rounded-md bg-gray-700 border ${
              startError ? 'border-red-500' : 'border-gray-500'
            } text-white focus:outline-none focus:ring-2 ${
              startError ? 'focus:ring-red-500' : 'focus:ring-blue-500'
            } w-full`}
          />
          <button
            onClick={() => {
              const fallbackEnd = endInput ? parseFloat(endInput) : currentTime + 5;
              if (currentTime >= fallbackEnd) {
                setStartError('Start must be less than End');
                return;
              }

              setStartError('');
              setNewFilterStart(currentTime);
              setStartInput(formatFloat(currentTime));

              if (!endInput) {
                setNewFilterEnd(currentTime + 5);
                setEndInput(formatFloat(currentTime + 5));
              }
            }}
            className="sm:w-auto w-full px-3 py-2 bg-blue-500 hover:bg-blue-600 rounded-md text-sm text-white focus:outline-none focus:ring-2 focus:ring-blue-400"
          >
            Set Current
          </button>
        </div>
        {startError && <p className="text-sm text-red-400 mt-1">{startError}</p>}

        {/* END INPUT */}
        <div className="flex flex-col sm:flex-row sm:items-center gap-2 w-full mt-3">
          <label htmlFor="new-end-time" className="text-gray-300 sm:w-20 w-full">End:</label>
          <input
            type="text"
            id="new-end-time"
            value={endInput ?? ''}
            placeholder="0.00"
            onChange={(e) => {
              setEndInput(e.target.value);
              setEndError('');
            }}
            onBlur={() => {
              const raw = endInput?.trim();
              if (!raw || !isValidFloat(raw)) {
                setEndError('Invalid time format');
                return;
              }

              const parsed = parseFloat(raw);
              const fallbackStart = startInput ? parseFloat(startInput) : parsed - 5;

              if (parsed <= fallbackStart) {
                setEndError('End must be greater than Start');
                return;
              }

              setEndInput(formatFloat(parsed));
              setNewFilterEnd(parsed);

              if (!startInput) {
                const start = parsed - 5;
                setNewFilterStart(start);
                setStartInput(formatFloat(start));
              }
            }}
            className={`flex-1 p-2 rounded-md bg-gray-700 border ${
              endError ? 'border-red-500' : 'border-gray-500'
            } text-white focus:outline-none focus:ring-2 ${
              endError ? 'focus:ring-red-500' : 'focus:ring-blue-500'
            } w-full`}
          />
          <button
            onClick={() => {
              const fallbackStart = startInput ? parseFloat(startInput) : currentTime - 5;
              if (currentTime <= fallbackStart) {
                setEndError('End must be greater than Start');
                return;
              }

              setEndError('');
              setNewFilterEnd(currentTime);
              setEndInput(formatFloat(currentTime));

              if (!startInput) {
                const start = currentTime - 5;
                setNewFilterStart(start);
                setStartInput(formatFloat(start));
              }
            }}
            className="sm:w-auto w-full px-3 py-2 bg-blue-500 hover:bg-blue-600 rounded-md text-sm text-white focus:outline-none focus:ring-2 focus:ring-blue-400"
          >
            Set Current
          </button>
        </div>
        {endError && <p className="text-sm text-red-400 mt-1">{endError}</p>}

          <div className="flex justify-end gap-3 mt-3">
            <button
              onClick={handleCancelAddFilter}
              className="py-2 px-4 bg-gray-500 hover:bg-gray-600 rounded-lg text-white font-semibold transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-gray-400 focus:ring-opacity-75"
            >
              Cancel
            </button>
            <button
              onClick={handleSaveFilter}
              className="py-2 px-4 bg-green-600 hover:bg-green-700 rounded-lg text-white font-semibold transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-opacity-75"
            >
              Save Filter
            </button>
          </div>
        </div>
      )}
                {/* Expandable Section for Managing Existing Filters */}
          {appliedFilters.length > 0 && ( // Only show if there are filters to manage
            <div className="w-full mt-4">
              <button
                onClick={() => setShowManageFilters(!showManageFilters)}
                className="w-full py-2 px-4 bg-gray-600 hover:bg-gray-700 rounded-lg text-white font-semibold transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-opacity-75 flex justify-between items-center"
              >
                <span>Manage Existing Filters ({appliedFilters.length})</span>
                <span>{showManageFilters ? '▲' : '▼'}</span>
              </button>

              {showManageFilters && (
                <div className="bg-gray-800 p-4 rounded-lg mt-2 max-h-60 overflow-y-auto custom-scrollbar"> {/* ⭐ ADDED custom-scrollbar CLASS HERE ⭐ */}
                  {appliedFilters.map((filter) => (
                    <div key={filter.id} className="flex flex-col md:flex-row items-center justify-between bg-gray-800 p-4 rounded-lg mb-3 last:mb-0 shadow-md border border-gray-700">
                      {editingFilterId === filter.id ? (
                        // START UPDATED EDIT FORM FOR INDIVIDUAL FILTER
                        <div className="flex-1 w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 items-center text-sm p-2 bg-gray-700 rounded-md">
                            <div className="col-span-full">
                                <label htmlFor={`edit-label-${filter.id}`} className="text-gray-300 sr-only">Label:</label>
                                <input
                                    type="text"
                                    id={`edit-label-${filter.id}`}
                                    value={editFilterLabel}
                                    onChange={(e) => setEditFilterLabel(e.target.value)}
                                    className="p-2 rounded-md bg-gray-600 border border-gray-500 text-white w-full focus:ring-blue-500"
                                    placeholder="Filter Label"
                                />
                            </div>

                            {/* Start time input */}
                            <div className="flex flex-col">
                                <label htmlFor={`edit-start-${filter.id}`} className="text-gray-400 text-xs mb-1">Start (s):</label>
                                <input
                                  type="text"
                                  id={`edit-start-${filter.id}`}
                                  value={editStartInput}
                                  onChange={(e) => {
                                    setEditStartInput(e.target.value);
                                    setEditStartError('');
                                  }}
                                  onBlur={() => {
                                    const raw = editStartInput.trim();
                                    if (!raw || !isValidFloat(raw)) {
                                      setEditStartError('Invalid time');
                                      return;
                                    }
                                    const parsed = parseFloat(raw);

                                    const endVal = editEndInput ? parseFloat(editEndInput) : editFilterEnd;

                                    if (parsed >= endVal) {
                                      setEditStartError('Start < End');
                                      return;
                                    }

                                    setEditStartInput(formatFloat(parsed));
                                    setEditFilterStart(parsed);
                                  }}
                                  min="0"
                                  max={duration}
                                  className={`p-2 rounded-md bg-gray-600 border ${
                                    editStartError ? 'border-red-500' : 'border-gray-500'
                                  } text-white w-full focus:ring-blue-500`}
                                />
                                {editStartError && (
                                  <p className="text-xs text-red-400 mt-0.5">{editStartError}</p>
                                )}
                            </div>

                            {/* End time input */}
                            <div className="flex flex-col">
                                <label htmlFor={`edit-end-${filter.id}`} className="text-gray-400 text-xs mb-1">End (s):</label>
                                <input
                                  type="text"
                                  id={`edit-end-${filter.id}`}
                                  value={editEndInput}
                                  onChange={(e) => {
                                    setEditEndInput(e.target.value);
                                    setEditEndError('');
                                  }}
                                  onBlur={() => {
                                    const raw = editEndInput.trim();
                                    if (!raw || !isValidFloat(raw)) {
                                      setEditEndError('Invalid time');
                                      return;
                                    }
                                    const parsed = parseFloat(raw);

                                    const startVal = editStartInput ? parseFloat(editStartInput) : editFilterStart;

                                    if (parsed <= startVal) {
                                      setEditEndError('End > Start');
                                      return;
                                    }

                                    setEditEndInput(formatFloat(parsed));
                                    setEditFilterEnd(parsed);
                                  }}
                                  min="0"
                                  max={duration}
                                  className={`p-2 rounded-md bg-gray-600 border ${
                                    editEndError ? 'border-red-500' : 'border-gray-500'
                                  } text-white w-full focus:ring-blue-500`}
                                />
                                {editEndError && (
                                  <p className="text-xs text-red-400 mt-0.5">{editEndError}</p>
                                )}
                            </div>

                            <div className="flex flex-col">
                                <label htmlFor={`edit-lens-${filter.id}`} className="text-gray-400 text-xs mb-1">Lens:</label>
                                <select
                                    id={`edit-lens-${filter.id}`}
                                    value={editFilterLensId}
                                    onChange={(e) => setEditFilterLensId(e.target.value)}
                                    className="p-2 rounded-md bg-gray-600 border border-gray-500 text-white w-full focus:ring-blue-500 custom-scrollbar"
                                >
                                    {availableLensOptions.length > 0 ? (
                                        availableLensOptions.map((lens) => (
                                            <option key={lens.id} value={lens.id}>{lens.name}</option>
                                        ))
                                    ) : (
                                        <option value="">No lenses available</option>
                                    )}
                                </select>
                            </div>
                        </div>
                      ) : (
                        // Display mode for individual filter
                        <div className="flex-1 w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 text-sm md:text-base">
                          <span className="text-purple-300 font-bold truncate text-lg">
                            {filter.label || 'Unnamed Filter'}
                          </span>
                          <span className="text-gray-200">
                            <span className="font-semibold">Start:</span> {formatTime(filter.start)}
                          </span>
                          <span className="text-gray-200">
                            <span className="font-semibold">End:</span> {formatTime(filter.end)}
                          </span>
                          <span className="text-gray-200 lg:text-right">
                            <span className="font-semibold">Lens:</span> {availableLensOptions.find(l => l.id === filter.filterId)?.name || 'Unknown'}
                          </span>
                        </div>
                      )}
                      <div className="flex flex-col sm:flex-row gap-2 mt-4 md:mt-0 md:ml-4">
                        {editingFilterId === filter.id ? (
                          <>
                            <button
                              onClick={handleSaveEditFilter}
                              className="py-2 px-4 bg-[#22C55E] hover:bg-[#4ADE80] rounded-lg text-sm text-white font-semibold transition-colors duration-200"
                              title="Save Changes"
                            >
                              Save
                            </button>
                            <button
                              onClick={handleCancelEditFilter}
                              className="py-2 px-4 bg-[#78716C] hover:bg-[#A8A29E] rounded-lg text-sm text-white font-semibold transition-colors duration-200" // "Cancel" button (edit mode)
                              title="Cancel Editing"
                            >
                              Cancel
                            </button>
                          </>
                        ) : (
                          <button
                            onClick={() => handleEditExistingFilter(filter)}
                            className="py-2 px-4 bg-[#A78BFA] hover:bg-[#C4B5FD] rounded-lg text-sm text-white font-semibold transition-colors duration-200" // "Edit" button (display mode)
                            title="Edit Filter"
                          >
                            Edit
                          </button>
                        )}
                        <button
                          onClick={() => handleDeleteExistingFilter(filter.id)}
                          className="py-2 px-4 bg-[#EF4444] hover:bg-[#F87171] rounded-lg text-sm text-white font-semibold transition-colors duration-200" // "Delete" button
                          title="Delete Filter"
                        >
                          Delete
                        </button>
                      </div>
                    </div>
                  ))}
                </div>
              )}
            </div>
          )}
        </div>
      );
    };

export default TimelineControls;
